...
async function git_checkout(
  ...
  try {
    ...
    console.log(
      `ERR523: Error by checkout ${repoPath}  for version  ${version}`
    );
    console.error(error);
  }
  return r_code;
...
async function git_custom(
  ...
  try {
    ...
    console.log(
      `ERR772: Error by git-command ${gitCommand}  on repo  ${repoPath}`
    );
    console.error(error);
  }
  return r_code;
  ...
  try {
    ...
    for (const repo in fyaml.repositories) {
      ...
      if (!Object.hasOwn(fyaml.repositories[repo], "type")) {
        console.log(
          `WARN390: Warning, the property "type" is missing for repo ${repo} !`
        );
...
class Subg {
  ...
  async discover_repos(
    ...
    if (!isPathAbsolute(this.discoverDir)) {
      if (
        !regex_pointSlash.test(this.discoverDir) &&
        this.discoverDir !== "."
      ) {
  ...
  async import_yaml(
    ...
    if (this.importYaml !== "") {
      ...
      console.log(
        `From imported Yaml, number of git-repos: ${
          Object.keys(this.listC).length
        }`
      );
      console.log(
        `From imported Yaml, number of excluded repos: ${list_non_git.length}`
      );
      for (const [idx, repoDir] of list_non_git.entries()) {
        console.log(
          `  ${(idx + 1)
            .toString()
            .padStart(3, " ")} - Excluded repo: ${repoDir}`
        );
      }
    }
    return r_code;
  ...
  async cd_checkout(): Promise<number> {
    ...
    for (const [idx, localPath] of list_cd.entries()) {
      const repo = this.listC[localPath];
      console.log(
        `===> ${idx + 1} - checkout  ${localPath}  at version  ${repo.version}`
      );
  ...
  async d_custom(
    ...
    for (const [idx, localPath] of repos.entries()) {
      console.log(
        `===> ${
          idx + 1
        } - On git-repo  ${localPath}  with command  git ${git_command}`
      );
